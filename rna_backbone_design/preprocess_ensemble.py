...
parser.add_argument(
    "--data_dir",
    help="Path to directory with RNA ensemble data.",
    type=str,
    default="data/rna_ensemble_data",
)
parser.add_argument(
    "--num_processes", help="Number of processes.", type=int, default=16
)
parser.add_argument(
...
parser.add_argument(
    "--verbose", help="Whether to print everything.", action="store_true"
)
...
def process_file(
    ...
    # NOTE: Modified from process_rna_pdb_files.py to write directly to write_dir
    # without creating subdirectories based on hash.
    ...
    for entity_id, group_chain_features in grouped_chains.items():
        for sym_id, chain_dict in enumerate(group_chain_features, start=1):
            new_all_chain_dict[f"{utils.int_id_to_str_id(entity_id)}_{sym_id}"] = (
                chain_dict
            )
    ...
    if complex_feats["bb_mask"].sum() < 1.0:
        ...
        return None
    assert len(complex_feats["bb_mask"]) == len(complex_feats["aatype"]), (
    ...
    interacting_res_mask = (
        inter_chain_mask
        & non_h_mask
        & (dist_mat <= inter_chain_interact_dist_threshold)
    )
...
def process_serially(
    file_path_list, write_dir_list, skip_existing=False, verbose=False
):
    all_metadata = []
    # Zip the paths and write dirs together
    for file_path, write_dir in tqdm(
        zip(file_path_list, write_dir_list), total=len(file_path_list)
    ):
        try:
...
def process_fn(file_path, write_dir, skip_existing=False, verbose=False):
    # This wrapper now needs to handle the write_dir logic if passed as a tuple/arg
    # However, Pool.map usually takes one arg. We can use starmap or partial.
    try:
        start_time = time.time()
        metadata = process_file(
            file_path, write_dir, skip_existing=skip_existing, verbose=verbose
...
def main(args):
    ...
    print(f"Found {len(clusters)} clusters. Collecting files...")
    ...
    for cluster_dir in clusters:
        ...
        # We do it here so process_file doesn't have to worry about the parent dir
        # although process_file does makedirs.

        pdb_files = list(structure_dir.glob("*.pdb"))
        for pdb_path in pdb_files:
            all_file_paths.append(str(pdb_path))
            all_write_dirs.append(str(feature_dir))

    total_num_paths = len(all_file_paths)
    print(f"Found {total_num_paths} PDB files to process.")

    write_dir_root = args.data_dir  # For metadata file
    if args.debug:
        metadata_file_name = "rna_metadata_debug.csv"
    else:
        metadata_file_name = "rna_metadata.csv"
    metadata_path = os.path.join(write_dir_root, metadata_file_name)
    ...
    if args.num_processes == 1 or args.debug:
        ...
        process_args = [
            (fp, wd, args.skip_existing, args.verbose)
            for fp, wd in zip(all_file_paths, all_write_dirs)
        ]

        with mp.Pool(processes=args.num_processes) as pool:
            all_metadata = pool.map(_process_wrapper, process_args)

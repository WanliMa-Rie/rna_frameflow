...
class EdgeEmbedder(nn.Module):
    def __init__(self, module_cfg):
        ...
        self.c_pair_in = self._cfg.c_pair_in
        ...
        self.pair_embedder = nn.Sequential(
            nn.LayerNorm(self.c_pair_in),
            nn.Linear(self.c_pair_in, self.c_p, bias=False),
        )

        total_edge_feats = self.feat_dim * 3 + self._cfg.num_bins * 2 + self.c_p
    ...
    def forward(self, s, t, sc_t, p_mask, pair_embedding):
        ...
        pair_emb = self.pair_embedder(pair_embedding)

        all_edge_feats = torch.concat(
            [cross_node_feats, relpos_feats, dist_feats, sc_feats, pair_emb], dim=-1
        )
